...
export default function Home() {
  ...
  // 🛡️ THE PERMANENT LIVE STYLING FIX: Forces the stylesheet directly into the browser DOM
  useEffect(() => {
    ...
    <main className="min-h-screen bg-slate-950 text-slate-100 flex flex-col items-center p-4 md:p-8 font-sans">
      <div className="w-full max-w-6xl flex flex-col sm:flex-row items-center justify-between mb-6 border-b border-slate-900 pb-4 gap-4">
        <div>
          ...
          <p className="text-[10px] font-mono text-slate-500 uppercase tracking-widest mt-1">Advanced ERP System: <span className="text-cyan-400 lowercase">{user.email}</span></p>
        </div>
      ...
      <div className="w-full max-w-6xl p-5 bg-slate-900 border border-slate-800 rounded-xl mb-6 grid grid-cols-1 md:grid-cols-3 gap-6 items-center">
        <div className="md:col-span-1">
          <h3 className="text-xs font-bold uppercase tracking-wider font-mono text-emerald-400">Profit Forecast Sandbox</h3>
          <p className="text-[11px] text-slate-500 mt-1">Slide markup targets to instantly predict total store gross profit realizations.</p>
          <div className="flex items-center gap-3 mt-3">
            <input type="range" min="10" max="70" value={targetMargin} onChange={e => setTargetMargin(Number(e.target.value))} className="w-full h-1 bg-slate-800 rounded-lg cursor-pointer accent-emerald-400" />
            <span className="text-xs font-mono font-bold bg-emerald-500/10 text-emerald-400 px-2 py-0.5 border border-emerald-500/20 rounded">{targetMargin}%</span>
          </div>
        </div>
        <div className="md:col-span-1 p-3 bg-slate-950 border border-slate-800 rounded-lg flex flex-col justify-center">
          <span className="text-[9px] font-mono text-slate-500 uppercase tracking-wider">Estimated Revenue Realization</span>
          <span className="text-xl font-extrabold text-slate-200 mt-1">K{estimatedRevenueForecast.toFixed(2)}</span>
        </div>
        <div className="md:col-span-1 p-3 bg-slate-950 border border-slate-800 rounded-lg flex flex-col justify-center border-l-2 border-l-emerald-500">
          <span className="text-[9px] font-mono text-emerald-400 uppercase tracking-wider font-bold">Projected Net Margin Return</span>
          <span className="text-xl font-extrabold text-emerald-400 mt-1">K{projectGrossProfitValue.toFixed(2)}</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 w-full max-w-6xl items-start">
        <div className="lg:col-span-1 bg-slate-900 rounded-2xl p-5 border border-slate-800 shadow-xl">
          ...
          <form onSubmit={handleAddProduct} className="space-y-4">
            <input type="text" required value={productName} onChange={e => setProductName(e.target.value)} placeholder="Item Name / Barcode Tag" className="w-full p-3 bg-slate-950 border border-slate-800 rounded-xl text-slate-200 text-xs font-medium focus:outline-none focus:border-emerald-400" />
            <input type="text" required value={category} onChange={e => setCategory(e.target.value)} placeholder="Category Segments" className="w-full p-3 bg-slate-950 border border-slate-800 rounded-xl text-slate-200 text-xs font-medium focus:outline-none focus:border-emerald-400" />
            <div className="grid grid-cols-2 gap-3">
              <input type="number" step="0.01" required value={price} onChange={e => setPrice(e.target.value)} placeholder="Price (Kina)" className="w-full p-3 bg-slate-950 border border-slate-800 rounded-xl text-gray-200 text-xs font-medium focus:outline-none focus:border-emerald-400" />
              <input type="number" required value={quantity} onChange={e => setQuantity(e.target.value)} placeholder="Quantity" className="w-full p-3 bg-slate-950 border border-slate-800 rounded-xl text-gray-200 text-xs font-medium focus:outline-none focus:border-emerald-400" />
            </div>
