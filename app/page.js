...
export default function Home() {
  ...
  // 📥 READ: Pull all rows from our Supabase favorites table
  ...
  // 📤 CREATE: Insert a selected track row into our database
  ...
  // 🗑️ DELETE: Wipe out a row matching a unique row ID
  const deleteTrackFromDatabase = async (id, title) => {
    ...
    try {
      const { error } = await supabase
        .from('favorites')
        .delete()
        .eq('id', id); // "eq" means match where the database column 'id' equals our target track id number

      if (error) throw error;

      setSaveStatus('Track removed from database cluster.');
      fetchSavedFavorites(); // Refresh our view instantly
      setTimeout(() => setSaveStatus(''), 3000);
  ...
  return (
    <main className="min-h-screen bg-slate-900 text-slate-100 flex flex-col items-center justify-start p-6 md:p-16 font-sans">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 w-full max-w-5xl items-start">
        ...
        {/* Left Side Panel: Live API Query Cards */}
        ...
        {/* Right Side Panel: Supabase Live Database Sync Panel */}
