...
export default function Home() {
  ...
  if (!user) {
    return (
      <>
        {/* NATIVE LINK INJECTION: Safely bypasses the Next.js server pre-render firewall */}
    ...
    <>
      ...
      <main className="min-h-screen bg-gray-950 text-gray-100 flex flex-col items-center p-4 md:p-8 font-sans">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 w-full max-w-6xl mb-6">
          ...
          <div className="p-4 bg-gray-900 border border-gray-800 rounded-xl"><span className="text-[9px] font-mono text-green-400 uppercase tracking-wider font-bold block">Total Value (+ GST)</span><span className="text-2xl font-extrabold text-green-400 block mt-1">K{totalValuationWithGST.toFixed(2)}</span></div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 w-full max-w-6xl items-start">
          <div className="lg:col-span-1 bg-gray-900 rounded-2xl p-5 border border-gray-800 shadow-xl">
            ...
            <form onSubmit={handleAddProduct} className="space-y-4">
              <input type="text" required value={productName} onChange={e => setProductName(e.target.value)} placeholder="Item Name / Barcode Tag" className="w-full p-3 bg-gray-950 border border-gray-800 rounded-xl text-gray-200 text-xs font-medium focus:outline-none focus:border-green-400" />
              <input type="text" required value={category} onChange={e => setCategory(e.target.value)} placeholder="Category Segments" className="w-full p-3 bg-gray-950 border border-gray-800 rounded-xl text-gray-200 text-xs font-medium focus:outline-none focus:border-green-400" />
              <div className="grid grid-cols-2 gap-3">
                <input type="number" step="0.01" required value={price} onChange={e => setPrice(e.target.value)} placeholder="Price (Kina)" className="w-full p-3 bg-gray-950 border border-gray-800 rounded-xl text-gray-200 text-xs font-medium focus:outline-none focus:border-green-400" />
                <input type="number" required value={quantity} onChange={e => setQuantity(e.target.value)} placeholder="Quantity" className="w-full p-3 bg-gray-950 border border-gray-800 rounded-xl text-gray-200 text-xs font-medium focus:outline-none focus:border-green-400" />
              </div>
          ...
          <div className="lg:col-span-2 bg-gray-900 bg-opacity-60 rounded-2xl p-5 border border-gray-800 shadow-xl">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
              ...
              <input type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="🔍 Filter inventory matrix item..." className="p-2 px-3 bg-gray-950 border border-gray-800 rounded-xl text-gray-200 text-xs font-medium w-full sm:w-64 focus:outline-none focus:border-green-400" />
            </div>
